...
ros::Publisher traj_pub, border_pub, pos_pub, start_pub, mode_pub, xy_pub, odom_pub, box_pub;
		...
		switch(state) {
			...
			case 3:
				...
				if(outBorder() || !freeXY) {
					...
					pos_cmd.x = odom.linear.x;
					pos_cmd.y = odom.linear.y;
					pos_cmd.z = 1.0;
					pos_pub.publish(pos_cmd);

					if(freeXY) {
						freeXY = false;
						ROS_INFO("Border broken!");
					}else {
						ROS_INFO("Esen trajectory completed!");
					}
					state = 2;
				}
				break;	
