...
int main(int argc, char** argv) {
	...
	ros::Subscriber mode_sub = nh.subscribe(q_name + "/cmd_mode", 1, modeCallback);			//Control XY axes mode
	...
	while(ros::ok()) {
		...
		//Wait for new transform data and grab new transform data
		listener.lookupTransform(asc.wframe_, asc.qframe_, ros::Time(0), qtransform);
